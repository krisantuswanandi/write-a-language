...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("wrong parameters. got=%d", len(fn.Parameters))
	}
	...
	if fn.Body.String() != expectedBody {
		t.Fatalf("wrong function body. got=%q", fn.Body.String())
	}
}
...
func TestArrayLiteral(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("wrong array length. got=%d", len(result.Elements))
	}
...
func TestHashLiteral(t *testing.T) {
	...
	if !ok {
		t.Fatalf("no hash found. got=%T", evaluated)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("wrong hash pairs. got=%d", len(result.Pairs))
	}
...
func testBooleanObject(t *testing.T, obj object.Object, expected bool) bool {
	...
	if result.Value != expected {
		t.Errorf("wrong boolean value. got=%t", result.Value)
		return false
	}
